...
- Enter **What to upload about** (for example `donut smp` or `drdonutt`) and Start Growth AI. The
  ...
  profile and same-origin profile item list before broader platform results, with a bounded set of verified
  official-profile posts for known creators whose older winners fall outside TikTok's short result window,
  plus YouTube Shorts as a cross-source fallback. Visible counters, metadata, and hydration data enforce the 50K-like floor. Likely picks
